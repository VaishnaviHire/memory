defmodule MemoryWeb.MainChannel do
  ...
  def handle_in("updates", %{"active1" => active1, "active2" => active2,
           "active1_pos" => active1_pos, "active2_pos" => active2_pos}, socket) do
    currState = socket.assigns[:currState]
     name = socket.assigns[:name]
     gameState  = Game.checks(currState, active1, active2, active1_pos, active2_pos)
  ...
  def handle_in("onsquareclick", %{"index" => index, "letter" => letter}, socket) do
     currState = socket.assigns[:currState]
     name = socket.assigns[:name]
     gameState = Game.show(currState, index, letter)
     socket = assign(socket, :currState, gameState)
     Memory.MemoryBackup.save(name, gameState)
     {:reply, {:ok, %{ "gameState" => gameState}}, socket}
  end
